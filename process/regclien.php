            ...
            if(consultasSQL::InsertSQL("cliente", "NIT, Nombre, NombreCompleto, Apellido, Direccion, Clave, Telefono, Email", "'$nitCliente','$nameCliente','$fullnameCliente','$apeCliente','$dirCliente', '$passCliente','$phoneCliente','$emailCliente'")){
                echo '<script>
                    swal({
                      ...
                      text: "Tu cuenta se ha construido correctamente, ya puedes ingresar",
                      type: "success",
               ...
               echo '<script>swal("ERROR", "Pisaste un ladrillo, inténtalo de nuevo", "error");</script>';
            }
        }else{
            echo '<script>swal("ERROR", "Ya tenemos un cliente con dicho DNI", "error");</script>';
        }
        mysqli_free_result($verificar);
    }else{
        echo '<script>swal("ERROR", "Las contraseñas no coinciden, inténtalo de nuevo", "error");</script>';
    }
}else {
