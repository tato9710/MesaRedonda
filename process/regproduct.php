<?php
    ...
    $edadProd=consultasSQL::clean_string($_POST['prod-edad']);
    $piezaProd=consultasSQL::clean_string($_POST['prod-pieza']);
    ...
    if($codeProd!="" && $nameProd!="" && $cateProd!="" && $priceProd!="" && $edadProd!="" && $piezaProd!="" && $stockProd!="" && $codePProd!=""){
            ...
            if($imgType=="image/jpeg" || $imgType=="image/png"){
                if(($imgSize/1024)<=$imgMaxSize){
                    ...
                    if(move_uploaded_file($_FILES['img']['tmp_name'],"../assets/img-products/".$imgFinalName)){
                        if(consultasSQL::InsertSQL("producto", "CodigoProd, NombreProd, CodigoCat, Precio, Descuento, Edad, Piezas, Stock, NITProveedor, Imagen, Nombre, Estado", "'$codeProd','$nameProd','$cateProd','$priceProd', '$descProd', '$edadProd','$piezaProd','$stockProd','$codePProd','$imgFinalName','$adminProd', '$estadoProd'")){
                            echo '<script>
                                swal({
